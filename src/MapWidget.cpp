...
QString MapWidget::exportBulkMarkersHtml(const QVector<BulkMarker>& markers, bool showTable) {
    ...
    QString html =
        ...
        "  maxZoom:19\n"
        "});\n"
        "var orthoLabels=L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/Reference/World_Boundaries_and_Places/MapServer/tile/{z}/{y}/{x}',{\n"
        ...
        "  if(e.layer===ortho){orthoLabels.addTo(map);}\n"
        "  else{map.removeLayer(orthoLabels);}\n"
        "});\n"
        "var data=" + json + ";\n"
